...
YangenWindow::YangenWindow( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	wxMenuItem* m_loadDiffuseMenuItem;
	m_loadDiffuseMenuItem = new wxMenuItem( m_fileMenu, wxID_LOAD_DIFFUSE, wxString( wxT("Load &Diffuse") ) + wxT('\t') + wxT("Ctrl+D"), wxT("Load diffuse only. Rest of the textures are autogenerated"), wxITEM_NORMAL );
	m_fileMenu->Append( m_loadDiffuseMenuItem );

	wxMenuItem* m_loadHeighmaptMenuItem;
	m_loadHeighmaptMenuItem = new wxMenuItem( m_fileMenu, wxID_LOAD_HEIGHTMAP, wxString( wxT("Load &Heightmap") ) + wxT('\t') + wxT("Ctrl+H"), wxT("Load heightmap only. Rest of the textures are autogenerated. There is no diffuse texture"), wxITEM_NORMAL );
	m_fileMenu->Append( m_loadHeighmaptMenuItem );

	wxMenuItem* m_loadDiffuseAndHeighmaptMenuItem;
	m_loadDiffuseAndHeighmaptMenuItem = new wxMenuItem( m_fileMenu, wxID_LOAD_DIFFUSE_AND_HEIGHTMAP, wxString( wxT("&Load diffuse && height maps") ) + wxT('\t') + wxT("Ctrl+O"), wxT("Load diffuse & height maps. only. Rest of the textures are autogenerated."), wxITEM_NORMAL );
	m_fileMenu->Append( m_loadDiffuseAndHeighmaptMenuItem );

	m_fileMenu->AppendSeparator();

	wxMenuItem* m_saveMenuItem;
	m_saveMenuItem = new wxMenuItem( m_fileMenu, wxID_SAVEFILE, wxString( wxT("&Save all textures") ) + wxT('\t') + wxT("Ctrl+S"), wxEmptyString, wxITEM_NORMAL );
	m_fileMenu->Append( m_saveMenuItem );

	wxMenuItem* m_saveMenuItem1;
	m_saveMenuItem1 = new wxMenuItem( m_fileMenu, wxID_SAVE_NORMAL_MAP, wxString( wxT("Save &Normal Map") ) + wxT('\t') + wxT("Ctrl+N"), wxEmptyString, wxITEM_NORMAL );
	m_fileMenu->Append( m_saveMenuItem1 );

	wxMenuItem* m_saveMenuItem2;
	m_saveMenuItem2 = new wxMenuItem( m_fileMenu, wxID_SAVE_ROUGHNESS_MAP, wxString( wxT("Save &Roughness map") ) + wxT('\t') + wxT("Ctrl+R"), wxEmptyString, wxITEM_NORMAL );
	m_fileMenu->Append( m_saveMenuItem2 );
	...
	// Connect Events
	m_fileMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( YangenWindow::OnMenuSelection ), this, m_loadDiffuseMenuItem->GetId());
	m_fileMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( YangenWindow::OnMenuSelection ), this, m_loadHeighmaptMenuItem->GetId());
	m_fileMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( YangenWindow::OnMenuSelection ), this, m_loadDiffuseAndHeighmaptMenuItem->GetId());
